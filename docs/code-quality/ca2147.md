...
1 つのオプションは、下の例の SecurityTransparentMethod メソッドをコード レビューすることです。そして、メソッドが昇格しても安全であると考えられる場合は、SecurityTransparentMethod をセキュアクリティカルとマークします。 そのためには、Assert のもとでメソッド内で発生するすべての呼び出しを含めて、メソッドに対して詳細で完全な、エラーのないセキュリティ監査を実行する必要があります。
