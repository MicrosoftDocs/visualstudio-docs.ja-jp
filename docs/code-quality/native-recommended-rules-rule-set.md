...
ms.date: 11/04/2016
description: Visual Studio の "ネイティブ推奨規則" 規則セットについて説明します。 ネイティブ コードのセキュリティ、堅牢性、およびその他の重要な問題の規則に関する説明を参照してください。
...
ネイティブ推奨規則は、ネイティブ コードで最も重大で一般的な問題 (潜在的なセキュリティ ホールやアプリケーションのクラッシュなど) に関するものです。 この規則セットには、"[ネイティブ最小規則](native-minimum-rules-rule-set.md)" 規則セット内のすべての規則が含まれます。

ネイティブ プロジェクトにカスタムの規則セットを作成する場合は、この規則セットを含めます。
