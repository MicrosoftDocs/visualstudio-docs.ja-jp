---
description: コンパイル単位がコンパイルされたプラットフォームの種類を取得します。
...
[出力] [CV_CPU_TYPE_e 列挙型](../../debugger/debug-interface-access/cv-cpu-type-e.md)に関するページにある列挙型から、コンパイル単位がコンパイルされたプラットフォームの種類を指定する値を返します。
