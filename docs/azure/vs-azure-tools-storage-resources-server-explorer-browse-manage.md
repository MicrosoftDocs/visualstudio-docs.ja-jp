...
**テーブル デザイナー** で、エンティティ (単一の行) またはプロパティ (単一のセル) のショートカット メニューを開き、 **[編集]** を選択します。
...
* **PartitionKey** と **RowKey** の値は編集できません。この操作は、Azure Table Storage ではサポートされていません。
...
* **テーブル デザイナー** に表示されているエンティティの一覧を更新するには、テーブル デザイナーで **[実行]** ボタンを選択します。
